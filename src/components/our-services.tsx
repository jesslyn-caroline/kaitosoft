...
function OurServices() {
    return (
        <section className={`w-full h-screen min-h-fit md:h-fit lg:h-screen px-6 sm:px-10 lg:px-24 pb-12 flex flex-col gap-4`}>
            <div>
                <motion.h2
            ...
            <motion.div 
                ...
                <motion.p 
                    variants={{hidden: { x: "30%", opacity: 0 }, show: { x: "0%", opacity: 1, transition: { duration: 1 } } }}
                    className={`text-md xl:text-lg 2xl:text-xl`}>
                    Solutions made to fit your needs. Choose what works best for you.
                </motion.p>
                <motion.div 
                    variants={{hidden: { x: "50%", opacity: 0 }, show: { x: "0%", opacity: 1, transition: { staggerChildren: 0.5, duration: 1 } }}}
                    viewport={{ once: true, amount: 0.3 }}
                    className={`mt-6 flex flex-col sm:grid sm:grid-cols-2 gap-4 lg:gap-6`}>
                    <motion.div 
                        variants={{hidden: { x: "50%", opacity: 0 }, show: { x: "0%", opacity: 1, transition: { duration: 1 } } }}
